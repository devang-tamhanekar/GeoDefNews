...
function fetchNews($rss_url, $limit = 6) {
    ...
    if (!$content) {
        echo "<p class='text-danger'>⚠️ Unable to load news feed.</p>";
        return;
    }

    $rss = @simplexml_load_string($content);
    if (!$rss) {
        echo "<p class='text-danger'>⚠️ Invalid RSS feed format.</p>";
        return;
    }
    ...
    foreach ($rss->channel->item as $item) {
        ...
        // Try to get image (media or enclosure)
        $image = "https://via.placeholder.com/350x200?text=Defence+News"; // fallback
...
// Example feed (you can replace or add more region-wise)
